 ...
 * made permanent. If it is closed or the process exits before the commit, the
 * old version of the file remains.
 */
public final class CommitableFile extends OutputStream implements Commitable {
     ...
     * @param commitOnClose true if the file should be automatically committed
     * when closed.
    ...
    /**
     * Close the file and commit the new version. The old version becomes a
     * backup
     */
    ...
    static void move(Path from, Path to) {
        try {
            ...
            System.out.println("Error moving }" + from + " to " + to + ": " + ex);
        }
    }
}
