// I didn't want to suck in all of Apache Commons for this one class
 ...
 * <p>
 * This class is conditionally thread-safe. The instance field for the maximum
 * code length is mutable {@link #setMaxCodeLen(int)} but is not volatile, and
 * accesses are not synchronized. If an instance of the class is shared between
 * threads, the caller needs to ensure that suitable synchronization is used to
 * ensure safe publication of the value between threads, and must not invoke
 * {@link #setMaxCodeLen(int)} after initial setup.
 * </p>
 *
 * @see <a href="http://drdobbs.com/184401251?pgno=2">Original Article</a>
 * @see
 * <a href="http://en.wikipedia.org/wiki/Metaphone">http://en.wikipedia.org/wiki/Metaphone</a>
 */
public class DoubleMetaphone {
    ...
    public String doubleMetaphone(String value, final boolean alternate) {
        ...
        while(!result.isComplete() && index <= value.length() - 1)
            index = switch(value.charAt(index)) {
                case 'A', 'E', 'I', 'O', 'U', 'Y' ->
                    handleAEIOUY(result, index);
                case 'B' ->
                    result.append('P', charAt(value, index + 1) == 'B' ? index + 2 : index + 1);
                case '\u00C7' -> // A C with a Cedilla ç Ç
                ...
                case 'F' ->
                    result.append('F', charAt(value, index + 1) == 'F' ? index + 2 : index + 1);
                case 'G' ->
                ...
                case 'K' ->
                    result.append('K', charAt(value, index + 1) == 'K' ? index + 2 : index + 1);
                case 'L' ->
                    handleL(value, result, index);
                case 'M' ->
                    result.append('M', conditionM0(value, index) ? index + 2 : index + 1);
                case 'N' ->
                    result.append('N', charAt(value, index + 1) == 'N' ? index + 2 : index + 1);
                case '\u00D1' ->
                    result.append('N', index + 1);
                case 'P' ->
                    handleP(value, result, index);
                case 'Q' ->
                    result.append('K', charAt(value, index + 1) == 'Q' ? index + 2 : index + 1);
                case 'R' ->
                ...
                case 'V' ->
                    result.append('F', charAt(value, index + 1) == 'V' ? index + 2 : index + 1);
                case 'W' ->
    ...
    /**
     * Check if the Double Metaphone values of two {@code String} values are
     * equal.
     *
     * @param value1 The left-hand side of the encoded
     * {@link String#equals(Object)}.
     * @param value2 The right-hand side of the encoded
     * {@link String#equals(Object)}.
    ...
    public class DoubleMetaphoneResult {
        ...
        public int append(final char value, int index) {
            append(value);
            return index;
        }
